...
class CommentsController < ApplicationController
  ...
  def update
    ...
    if @comment.update(comment_params)
      json_response({ comment: @comment, comments: Post.author_comments_json(@comment.post.comments) })
    else
      json_response({ errors: @comment.errors.full_messages }, 401)
    end
  end
  ...
  def set_post
    @post = Post.find(params[:comment][:post_id]) if params[:comment]&.dig(:post_id)
  end
